...
int main()
{
  ...
  printnum(&comkeyB);
  randomgenmod(&dhprime,&cleartext);
  printf("Alice has a clear text: \n");
  printnum(&cleartext);
  elgamalencrypt(&dhprime,&comkeyA,&cleartext,&cyphertext);
  printf("Alice sends her cyphertext to Bob using the common key: \n");
  printnum(&cyphertext);
  elgamaldecrypt(&dhprime,&comkeyB,&cyphertext,&decrypted);
  printf("Bob decrypts the cyphertext from Alice: \n");
  printnum(&decrypted);
}  // end main
