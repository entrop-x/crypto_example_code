...
number modulo;  // warning: lsd first !
...
int main()
{
  ...
  b.digit[0] = 2;
  b.digit[nofdigits - 3] = 2;
  printf("In ring modulo : \n");
  printnum(&modulo);
  
  for (trial = 0 ; trial < noftrials ; trial++)
    {
    if (trial % 10 == 1) 
      {
      printf("Trial %d.\n",trial);
    ...
    findnextprimefermat(&b,&nextprimef,6);
    findnextprimerabin(&b,&nextprimer,3);
    // printnum(&nextprimer);
    if (isbigger(&nextprimef, &nextprimer) != 0)
      {
      printf("Error at trial %d: b is : \n",trial);
      printnum(&b);
      printf("ERROR: next prime not the same for two primality tests ! \n");
      printf("the first prime number after b according to Fermat is : \n");
      printnum(&nextprimef);
      printf("the first prime number after b according to Rabin is : \n");
      printnum(&nextprimer);
      }
    sumnum(&nextprimer,&modulo,&b);
    }
}  // end main
...
/* applies the Fermat primality test to the number x with k trials randomly selected in the range 2..x-1 */
  ...
  do
    ...
    //printf("Test number was : \n");
    //printnum(&testa);
    //printf("And the Fermat test gave: \n");
    //printnum(&testresult);
...
/* applies the Miller-Rabin primality test to the number x with k trials randomly selected in the range 2..x-1 */
  ...
  while (r == 0);
  u--;
  // printf("miller rabin preparation: r is %d and there is still \n",u);
  // printnum(&rabin);
